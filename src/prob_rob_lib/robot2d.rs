...
impl Control {
    ...
    pub fn process(&self, cur: &State2d, dt: f64, noisy: bool) -> State2d {
        ...
        if noisy {
            ...
            let nd = Normal::new(v, v / 20.0).unwrap(); // 20% noise
            v = nd.sample(&mut rand::thread_rng());
            let nd = Normal::new(0.0, deg2rad!(10.0)).unwrap(); // 10deg noise
            dir_err = nd.sample(&mut rand::thread_rng());
            let nd = Normal::new(w, w / 20.0).unwrap();
...
pub fn draw_arrow(
    ...
    .points(
        &[x],
        &[y],
        &[caption, color, PointSymbol('O'), PointSize(0.5)],
    );
}

pub fn draw_robot_animation(robot: &mut Robot2d, end_time: f64, create_gif: bool, name: &str) {
    let mut fg = Figure::new();
    if create_gif {
        fg.set_terminal(
            "gif animate optimize delay 2 size 480,360",
            &("movie/".to_owned() + name + ".gif"),
        );
    }
